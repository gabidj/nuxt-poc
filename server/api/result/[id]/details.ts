...
export default defineEventHandler(async (event) => {
    try {
        ...
        const detailsFilePath = path.join(process.cwd(), 'public', 'data', 'uploads', id, 'details.json')

        // Check if the quiz file exists
        try {
            await fs.access(detailsFilePath)
        } catch {
            throw createError({
                statusCode: 404,
        ...
        try {
            const quizContent = await fs.readFile(detailsFilePath, 'utf-8')
